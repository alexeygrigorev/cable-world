...
- **100% = the game.** The editor and Godot use the same fixed map scale at
  100%, independent of viewport size. Resizing the window changes how much map
  is visible, not the size of hexes, glyphs, or labels. Opens at 100% on
  Germany; zoom 50–200% (step 25%) via the `+/−` buttons. Wheel/trackpad pans.
