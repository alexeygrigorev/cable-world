...
func _ready() -> void:
	...
	object_mode_button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	object_mode_button.pressed.connect(_on_object_mode_pressed)
	rows.add_child(object_mode_button)
...
func show_empty_state() -> void:
	...
	object_mode_button.disabled = true
...
func show_object(object_data: Dictionary, can_register_photo: bool = false) -> void:
	...
	object_mode_button.disabled = false
...
func _on_object_mode_pressed() -> void:
	if current_object.is_empty():
		return
