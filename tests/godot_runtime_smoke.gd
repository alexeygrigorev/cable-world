		...
		if row.get_child_count() == 1:
			...
			if row_content.get_child_count() == 5:
				...
				if text_box.get_child_count() == 3:
					...
					var type_label := text_box.get_child(1) as Label
					var meta_label := text_box.get_child(2) as Label
					...
					_expect(type_label != null and type_label.text.contains("канатная"), "Object row must show the transport type separately from status metadata.", failures)
