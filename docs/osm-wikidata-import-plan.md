...
Review проходит только в staging JSON и не меняет SQLite-схему. У каждой записи есть `review.state`:

- `candidate` - новая или неполная запись. Ее можно хранить и повторно проверять, но нельзя включать в seed или preview seed.
- `approved` - запись прошла ручную проверку и может попасть в preview JSON/SQL для будущего seed.
- `rejected` - запись явно отклонена как нерелевантная, дубль или недостаточно надежная. Она не попадает в seed, но остается в staging для аудита решения.

Чеклист ручной проверки перед переводом в `approved`:

- русский `localized.ru.title` понятен семье и не является машинным названием из Wikidata;
- русский `localized.ru.description` описывает объект, а не только повторяет тип или город;
- `transport_type_id` выбран из текущего справочника `transport_types`, а не оставлен как черновая догадка OSM/Wikidata;
- `latitude` и `longitude` указывают на полезную точку объекта на карте, а не на офис оператора, отдельную станцию или случайный центр relation;
- `operator` заполнен человеческим названием оператора, если оператор известен из OSM/Wikidata или официального сайта;
- `source_urls` содержит хотя бы одну проверяемую ссылку на OSM, Wikidata или официальный сайт, достаточную для трассировки решения;
- `operational_status` проверен отдельно от статуса посещения и не выводится автоматически из `website=*`, `P856` или lifecycle-тегов.

Правило для эксплуатационного статуса строгое: `operational_status` остается `unknown`, если нет свежего официального источника оператора или владельца с расписанием, сообщением о работе, закрытии или сезонности. Если статус отличается от `unknown`, reviewer заполняет `status_checked_at`, `status_source_url` и `status_note`; ссылка должна вести на официальный источник, а не только на OSM/Wikidata.

Генерация seed допускает только `review.state = "approved"`. Для локальной проверки используется `scripts/import_review_preview.py`: скрипт читает staging JSON как одиночную запись, массив записей, объект `{ "records": [...] }` или collector payload `{ "candidates": [...] }`, валидирует approved-записи по чеклисту, генерирует preview JSON/SQL и не пишет в production SQLite. Записи `candidate` и `rejected` игнорируются при генерации, даже если остальные поля заполнены.
