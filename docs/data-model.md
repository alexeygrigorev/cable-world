...
Object mode использует цепочку выбранный `TransportObject` -> детальная схема -> станция -> поездка. Обязательные поля для `ObjectStation`, `RouteDirection`, `RouteSegment` и `EngineeringPoint` описаны в этом документе и проверяются тестами/seed-данными, без отдельного markdown-контракта.
