...
Минимальные V3 requirements и UI/data contract для задач #32 «инженерный режим объекта» и #34 «режим прогулки по объекту» закреплены в [v3-scene-contract.md](v3-scene-contract.md). Этот контракт является post-1.0 roadmap и не blocker для пользовательской цели 1.0.
