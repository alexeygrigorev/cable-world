...
Canonical production doc: [pipelines/city-glyphs.md](pipelines/city-glyphs.md).

Historical note: this handoff originally contained the old single-landmark city sheet and the first high-resolution cluster experiments. The active process is now the high-resolution multi-symbol city cluster pipeline only. It includes `map_pipeline.sheet_slice_audit`, component-based sheet slicing, `1024x1024` normalized transparent city sprites, and runtime outlined assets under `assets/sprites/city_landmark_clusters_hi_res/outlined`.
