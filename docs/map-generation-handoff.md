...
## Harz production glyph-layer pass

Issue #69 first production-art slice replaces the generic Harz placeholder with a reusable transparent source layer generated by the local renderer, not by a monolithic map bitmap.

No external raster generation was used for this pass, so there is no imagegen prompt. The repeatable workflow is:
...
- `assets/map/germany_styled.png`
- `assets/map/massifs/harz.png`
- `assets/map/massifs/harz.json`
- `assets/map/massifs/manifest.json`

Implementation notes:

- `RELIEF_REGIONS["harz"]` uses `custom_renderer: "harz_production_v1"`.
- The layer is composed from existing transparent atlas glyphs and larger tree masses: `highland_forest_1`, `highland_forest_2`, and `atlas_forest_pine_dense`.
- `ridge_bands` remain in metadata as real Harz structure, but the renderer no longer draws the old generic green ridge-band rectangle into the Harz source layer.
- `map_pipeline/data/terrain_massif_layers.json` marks Harz as `production_candidate_custom_harz_v1`.

Self-audit: this is a material #69 improvement because Harz is now a cleaner reusable terrain layer instead of a generic stamped mountain group with visible technical bands. It is not a full `10/10` terrain solution: Alps and other massifs still need equivalent custom/elevation-backed passes and strict screenshot review.
