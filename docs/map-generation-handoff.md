...
Issue #68/#69 Alpine visibility slice, 2026-06-01:

- `map_pipeline.compose_map` now renders the Alps as a stronger cross-border massif with a visible `german_edge_alpine_wall` ridge band, larger German Alpine edge glyphs and foothill arcs.
- `northern_italy_alps_massif` is a separate reusable source layer, not a monolithic underlay, so the Alps continue south of Austria/Switzerland and can later be replaced by DEM-derived art.
- Source layers and metadata are regenerated by the same command:
  ```bash
  uv run python -m map_pipeline.compose_map
  ```
- This is a production terrain glyph-layer slice, but still pre-DEM. It improves readability and layer structure; it does not close the final `10/10` Alpine accuracy requirement until DEM/ridge masks are produced and strict screenshots pass.
